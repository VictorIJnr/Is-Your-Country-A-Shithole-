...
router.get("/", function(req, res, next) {
  ...
  if (req.query.country) {
    if (
      req.query.country.toLowerCase() === "united states of america" ||
      req.query.country.toLowerCase() === "america"
    ) {
      var result = {
        header:
          "No, Donald Trump doesn't think " +
          req.query.country +
          " is a shithole.",
    ...
    shithole.isShithole(req.query.country, function(data) {
      ...
      if (req.query.country.toLowerCase() === "russia") {
        result.header = "[Error, this page has been hacked]";
        result.bodyApprehended = "";
        result.tagApprehended = "";

        result.bodyImmigration = "";
        result.tagImmigration = "";
      ...
      result.bodyApprehended =
        "Number of people apprehended 2007-2016: " + data.apprehended;
      result.tagApprehended = data.apprehendedTag;

      result.bodyImmigration =
        "Number of people who entered the country 2007-2016: " + data.total;
      result.tagImmigration = data.totalTag;

      result.bodyGdp =
        "Country's Average GDP per captia 2007-2016: " + data.gdp + " USD";
